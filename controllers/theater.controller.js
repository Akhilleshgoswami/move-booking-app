...
const checkMovie = async (req, res) => {
 ...
 try {
  const result = await TheaterService.checkMovieInAtheater(req.params.theaterId, req.params.movieId)
  if (result.err) {
   errorResponseBody.error = result.err
   return res.status(result.code).json(errorResponseBody)
  }
...
module.exports = {
 create,
 getTheater, deleteTheater, fetchTheater, updateMovies, fetchAllTheater, getMovies,checkMovie
}
