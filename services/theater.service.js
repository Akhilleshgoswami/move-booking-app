...
const checkMovieInAtheater = async (theaterId, movieId) => {
 try {
  const theater = await Theater.findById(theaterId)
  if (!theater) {
   return {
  ...
  return theater.movies.indexOf(movieId) != -1;
...
module.exports = {
 createTheater,
 getDocById,
 deleteTheaterd, fetchTheater, updateMoviesInTheaters, getAllTheaters, getMoviesInATheater,checkMovieInAtheater
}
